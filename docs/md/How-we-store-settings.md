...
In portable version the file is stored in the same directory as the SumatraPDF+ executable. In non-portable version, it's in `%LOCALAPPDATA%\SumatraPDF+` directory.
